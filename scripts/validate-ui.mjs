...
const requiredFiles = [
  'interaction-system.css','interaction-system.js','landmark-visibility.js','apple-controls.css','apple-controls-loader.js','apple-controls.js','ui-state.js','ui-adapters.js','search-index.js','history-catalog.js','history-engine.js','history-engine.css','history-presence-r14.js','history-presence-r14.css','mobile-search-snap-r15.js','mobile-search-snap-r15.css','r9-polish.css','earth-history.css','earth-history.js','earth-era-context.js','earth-ui-sync.css','earth-ui-sync.js','life-regions-r12.css','r12-ui.js','life-evidence.js','netlify/functions/place-summary.js','netlify/functions/paleocoastlines.js'
];
const errors = [];
for (const path of requiredFiles) { try { await access(path); } catch { errors.push(`Missing required UI file: ${path}`); } }

const [html,bootstrap,netlify,version,appleControls,appleLoader,uiState,historyEngine,historyStyle,historyPresence,historyPresenceStyle,mobileRuntime,mobileStyle,earthHistory,r12Ui,lifeRuntime] = await Promise.all([
  readFile('index.html','utf8'),readFile('bootstrap.js','utf8'),readFile('netlify.toml','utf8'),readFile('version.json','utf8'),readFile('apple-controls.js','utf8'),readFile('apple-controls-loader.js','utf8'),readFile('ui-state.js','utf8'),readFile('history-engine.js','utf8'),readFile('history-engine.css','utf8'),readFile('history-presence-r14.js','utf8'),readFile('history-presence-r14.css','utf8'),readFile('mobile-search-snap-r15.js','utf8'),readFile('mobile-search-snap-r15.css','utf8'),readFile('earth-history.js','utf8'),readFile('r12-ui.js','utf8'),readFile('life-evidence.js','utf8')
]);
...
requireText(mobileRuntime,'navigator.vibrate','Milestone snap feedback is missing');
requireText(mobileStyle,'--wl-search-top','Keyboard-safe search positioning is missing');
requireText(mobileStyle,'--wl-search-results-height','Bounded mobile suggestion height is missing');
requireText(mobileStyle,'.timeline-snap-label','Milestone snap feedback is not styled');
...
for (const source of [bootstrap,mobileRuntime]) if(!source.includes(BUILD)) errors.push(`An r15 runtime is missing the ${BUILD} marker`);
if(errors.length){ console.error('UI validation failed:'); errors.forEach((error)=>console.error(`- ${error}`)); process.exit(1); }
console.log('Unified UI, keyboard-safe search, milestone snapping, history presence, regional life, and place-sheet wiring are valid.');
