...
const requiredFiles = [
  ...
  'history-engine.js',
  'history-engine.css',
  'history-presence-r14.js',
  'history-presence-r14.css',
...
const [catalogSource, engine, style, presence, presenceStyle, bootstrap, versionSource, framework] = await Promise.all([
  ...
  readFile('history-presence-r14.js', 'utf8'),
  readFile('history-presence-r14.css', 'utf8'),
...
try { vm.runInNewContext(catalogSource, context, { filename: 'history-catalog.js' }); }
catch (error) { failures.push(`History catalog does not execute: ${error.message}`); }
...
requireText(presenceStyle, 'body.timeline-active .history-presence-card', 'The card does not yield to active surfaces');
...
if (!bootstrap.includes(BUILD) || !presence.includes(BUILD)) failures.push(`The r14 production runtime is missing ${BUILD}`);
