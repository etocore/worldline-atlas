...
const [runtime,style,proxy,netlify,bootstrap,html,version,documentation] = await Promise.all([
  readFile('earth-history.js','utf8'),readFile('earth-history.css','utf8'),readFile('netlify/functions/paleocoastlines.js','utf8'),readFile('netlify.toml','utf8'),readFile('bootstrap.js','utf8'),readFile('index.html','utf8'),readFile('version.json','utf8'),readFile('docs/EARTH_ENGINE.md','utf8')
]);
...
let parsedVersion; try { parsedVersion = JSON.parse(version); } catch { failures.push('version.json is not valid JSON'); }
...
console.log('Earth History foundation remains wired and scientifically bounded in r15.');
