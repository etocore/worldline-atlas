...
const failures = [];
for (const file of requiredFiles) { try { await access(file); } catch { failures.push(`Missing cached Earth or life file: ${file}`); } }

const [bootstrap,cacheRuntime,lifeRuntime,lifeStyle,r12Ui,timelineStyle,timelineRuntime,builder,workflow,manifestSource,versionSource,netlify] = await Promise.all([
  readFile('bootstrap.js','utf8'),readFile('earth-cache.js','utf8'),readFile('life-evidence.js','utf8'),readFile('life-regions-r12.css','utf8'),readFile('r12-ui.js','utf8'),readFile('apple-timeline-r11.css','utf8'),readFile('apple-timeline-r11.js','utf8'),readFile('scripts/cache-earth-data.mjs','utf8'),readFile('.github/workflows/cache-earth-data.yml','utf8'),readFile('data/earth/cache/manifest.json','utf8'),readFile('version.json','utf8'),readFile('netlify.toml','utf8')
]);
...
if(bootstrap.indexOf("loadScript('earth-cache.js')")>bootstrap.indexOf("loadScript('earth-history.js')")) failures.push('The Earth cache loads after the Earth history runtime');
...
for (const icon of ['leaf','fish','lizard','paw','shell']) requireText(lifeRuntime,`${icon}: {`,`The ${icon} life image is missing`);
...
if(lifeRuntime.includes('life-evidence-cluster-count')||lifeRuntime.includes('point_count_abbreviated')) failures.push('Numbered fossil cluster bubbles remain in the default experience');
...
let manifest; try { manifest=JSON.parse(manifestSource); } catch { failures.push('Earth cache manifest is not valid JSON'); }
if(manifest){ if(manifest.model!=='CAO2024') failures.push('Earth cache manifest does not identify CAO2024'); if(!Array.isArray(manifest.life)||manifest.life.length<3) failures.push('The fossil manifest is incomplete'); for(const age of [66,100,250]) if(!manifest.life.some((entry)=>Number(entry.ageMa)===age)) failures.push(`Missing fossil data for ${age} Ma`); }
...
for(const runtime of [lifeRuntime,r12Ui]) if(!runtime.includes(LIFE_BUILD)) failures.push(`An r12 life runtime is missing the ${LIFE_BUILD} marker`);
if(failures.length){ console.error('Cached Earth and life-region validation failed:'); failures.forEach((failure)=>console.error(`- ${failure}`)); process.exit(1); }
console.log('GitHub-cached Earth, illustrated life regions, evidence disclosure, and shared visual foundation are valid in r16.');
